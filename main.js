...
function injectCodeExecutionUI(webContents) {
  ...
  s += "if(!document.getElementById('ds-run-styles')){const st=document.createElement('style');st.id='ds-run-styles';st.textContent='.ds-run-btn{background:#22c55e!important;color:white!important;border:none!important;border-radius:4px!important;padding:4px 10px!important;font-size:12px!important;cursor:pointer!important;display:inline-flex!important;align-items:center!important;gap:4px!important;transition:background 0.2s!important;margin-left:8px!important;margin-right:4px!important;font-family:system-ui,-apple-system,sans-serif!important;position:relative!important;z-index:100!important;vertical-align:middle!important}.ds-run-btn:hover{background:#16a34a!important}.ds-run-btn:disabled{background:#6b7280!important;cursor:not-allowed!important}.ds-edit-btn{background:#3b82f6!important;color:white!important;border:none!important;border-radius:4px!important;padding:4px 10px!important;font-size:12px!important;cursor:pointer!important;display:inline-flex!important;align-items:center!important;gap:4px!important;transition:background 0.2s!important;margin-right:4px!important;font-family:system-ui,-apple-system,sans-serif!important;position:relative!important;z-index:100!important;vertical-align:middle!important}.ds-edit-btn:hover{background:#2563eb!important}.ds-reset-btn{background:#6b7280!important;color:white!important;border:none!important;border-radius:4px!important;padding:4px 10px!important;font-size:12px!important;cursor:pointer!important;display:inline-flex!important;align-items:center!important;gap:4px!important;transition:background 0.2s!important;margin-right:8px!important;font-family:system-ui,-apple-system,sans-serif!important;position:relative!important;z-index:100!important;vertical-align:middle!important}.ds-reset-btn:hover{background:#4b5563!important}.ds-code-edit-area{width:100%!important;background:#1e1e1e!important;color:#d4d4d4!important;border:1px solid #333!important;border-radius:4px!important;font-family:monospace!important;font-size:13px!important;padding:10px!important;outline:none!important;resize:vertical!important;min-height:60px!important;box-sizing:border-box!important;display:block!important}textarea._27c9245,textarea[placeholder*=\"DeepSeek\"]{resize:none!important;height:100%!important;max-height:none!important;min-height:48px!important;overflow-y:auto!important;z-index:10!important;border:none!important;background:transparent!important}.ds-resizer-container{position:relative!important;padding-top:4px!important;height:52px!important;min-height:48px!important;max-height:none!important;display:flex!important;flex-direction:column!important;transition:none!important;overflow:visible!important;--container-height:52px}.ds-top-resizer{position:absolute!important;top:0!important;left:0!important;right:0!important;height:3px!important;background:#5686fe!important;cursor:ns-resize!important;z-index:9999!important;border-radius:2px 2px 0 0!important;opacity:0.8!important}.ds-top-resizer:hover{height:6px!important;opacity:1!important;box-shadow:0 0 8px rgba(86,134,254,0.6)!important}';document.head.appendChild(st);}";
  ...
  s += "function addRunButton(pre,language,code){if(pre.getAttribute('data-ds-processed'))return;pre.setAttribute('data-ds-processed','true');pre.setAttribute('data-ds-original',code);const btnContainer=document.createElement('div');btnContainer.style.display='inline-flex';btnContainer.style.alignItems='center';btnContainer.style.verticalAlign='middle';const runBtn=document.createElement('button');runBtn.className='ds-run-btn';runBtn.innerHTML='▶ Run';runBtn.title='Execute '+language;const editBtn=document.createElement('button');editBtn.className='ds-edit-btn';editBtn.innerHTML='✎ Edit';editBtn.title='Edit code';const resetBtn=document.createElement('button');resetBtn.className='ds-reset-btn';resetBtn.innerHTML='↺ Reset';resetBtn.title='Restore original code';let isEditing=false;let editor=null;editBtn.addEventListener('click',function(e){e.preventDefault();e.stopPropagation();isEditing=!isEditing;if(isEditing){editBtn.innerHTML='✔ Done';editBtn.style.setProperty('background','#10b981','important');const codeEl=pre.querySelector('code')||pre;editor=document.createElement('textarea');editor.className='ds-code-edit-area';editor.value=codeEl.textContent;editor.style.height=(pre.offsetHeight+20)+'px';pre.style.display='none';pre.parentNode.insertBefore(editor,pre.nextSibling);}else{editBtn.innerHTML='✎ Edit';editBtn.style.setProperty('background','#3b82f6','important');if(editor){const codeEl=pre.querySelector('code')||pre;codeEl.textContent=editor.value;editor.remove();editor=null;}pre.style.display='block';}},true);resetBtn.addEventListener('click',function(e){e.preventDefault();e.stopPropagation();const originalCode=pre.getAttribute('data-ds-original');const codeEl=pre.querySelector('code')||pre;codeEl.textContent=originalCode;if(isEditing&&editor)editor.value=originalCode;},true);runBtn.addEventListener('click',async function(e){e.preventDefault();e.stopPropagation();let codeToRun=(pre.querySelector('code')||pre).textContent;if(isEditing&&editor)codeToRun=editor.value;runBtn.innerHTML='⏳ Running...';runBtn.disabled=true;try{const execLang=language.toLowerCase()==='html'?'javascript':language;await window.__executeCodeAndShowModal(codeToRun,execLang);}catch(err){console.error('Execution failed:',err);}runBtn.innerHTML='▶ Run';runBtn.disabled=false;},true);btnContainer.appendChild(runBtn);btnContainer.appendChild(editBtn);btnContainer.appendChild(resetBtn);function findToolbar(pre){var lang='';var cm=(pre.querySelector('code')||pre).className||'';var lm=cm.match(/language-([a-zA-Z0-9]+)/);if(lm)lang=lm[1].toLowerCase();var first=null;for(var el=pre,d=0;el&&d<3;d++,el=el.parentElement){for(var i=0;i<el.children.length;i++){var c=el.children[i];if(c===pre||c.contains(pre)||!c.querySelector)continue;if(!c.querySelector('button')&&!c.querySelector('[role=button]'))continue;if(!first)first=c;if(lang&&c.textContent&&c.textContent.toLowerCase().indexOf(lang)>=0)return c;}}return first;}let toolbar=findToolbar(pre);";
  ...
  s += "else{const wrapper=document.createElement('div');wrapper.style.cssText='display:flex;align-items:center;gap:8px;padding:4px 8px;background:rgba(0,0,0,0.3);border-radius:6px 6px 0 0;margin-bottom:-1px;';wrapper.appendChild(btnContainer);pre.parentElement.insertBefore(wrapper,pre);}}";
