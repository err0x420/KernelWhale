...
function injectCodeExecutionUI(webContents) {
  ...
  s += "function addRunButton(pre,language,code){if(pre.getAttribute('data-ds-processed'))return;pre.setAttribute('data-ds-processed','true');const btn=document.createElement('button');btn.className='ds-run-btn';btn.innerHTML='▶ Run';btn.title='Execute '+language;btn.addEventListener('click',async function(e){e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();btn.innerHTML='⏳ Running...';btn.disabled=true;try{const execLang=language.toLowerCase()==='html'?'javascript':language;await window.__executeCodeAndShowModal(code,execLang);}catch(err){console.error('Execution failed:',err);}btn.innerHTML='▶ Run';btn.disabled=false;},true);let toolbar=null;const parent=pre.parentElement;if(parent){for(const child of parent.children){if(child!==pre&&child.querySelector&&child.querySelector('button')){toolbar=child;break;}}}";
  s += "if(toolbar){const langContainer=toolbar.querySelector('.d2a24f03');if(langContainer){const langSpan=langContainer.querySelector('.d813de27');if(langSpan){if(langSpan.nextSibling)langContainer.insertBefore(btn,langSpan.nextSibling);else langContainer.appendChild(btn);}else langContainer.appendChild(btn);}else toolbar.appendChild(btn);}";
  s += "else{const wrapper=document.createElement('div');wrapper.style.cssText='display:flex;align-items:center;gap:8px;padding:4px 8px;background:rgba(0,0,0,0.3);border-radius:6px 6px 0 0;margin-bottom:-1px;';const langLabel=document.createElement('span');langLabel.className='ds-lang-label';langLabel.textContent=language;langLabel.style.cssText='font-size:12px;font-weight:600;color:#9ca3af;text-transform:uppercase;';wrapper.appendChild(langLabel);wrapper.appendChild(btn);pre.parentElement.insertBefore(wrapper,pre);}}";
  ...
  s += "function setupResizer(){const textarea=document.querySelector('textarea._27c9245')||document.querySelector('textarea[placeholder*=\"DeepSeek\"]');if(!textarea)return;const container=textarea.parentElement;if(!container||container.querySelector('.ds-top-resizer'))return;container.classList.add('ds-resizer-container');const resizer=document.createElement('div');resizer.className='ds-top-resizer';container.prepend(resizer);let startY,startHeight;resizer.addEventListener('mousedown',(e)=>{startY=e.clientY;startHeight=container.offsetHeight;document.body.style.userSelect='none';document.body.style.cursor='ns-resize';const onMouseMove=(moveEvent)=>{const dy=startY-moveEvent.clientY;let newHeight=startHeight+dy;if(newHeight>=50&&newHeight<=600){container.style.setProperty('height',newHeight+'px','important');container.style.setProperty('--container-height',newHeight+'px');}};const onMouseUp=()=>{document.removeEventListener('mousemove',onMouseMove);document.removeEventListener('mouseup',onMouseUp);document.body.style.userSelect='';document.body.style.cursor='';};document.addEventListener('mousemove',onMouseMove);document.addEventListener('mouseup',onMouseUp);e.preventDefault();e.stopPropagation();});}setInterval(setupResizer,1000);setInterval(processCodeBlocks,2000);})();";
